      ...
      <div className="flex justify-center gap-1.5 mt-3">
        {products.map((_, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
            className={`w-2 h-2 rounded-full ${i === active ? 'bg-accent' : 'bg-gray-300'}`}
          />
        ))}
      </div>
    </div>
      ...
      <Reveal as="section" animation="zoom-in" className="relative py-10 bg-[rgb(58,88,129)] overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto px-6 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-2.5">
            <Link href="/contact" className="btn-pill btn-unity text-xs">
              Talk to Engineering <ArrowRight className="w-3.5 h-3.5" />
            </Link>
            <Link href="/process" className="btn-pill text-xs bg-transparent text-white border border-white/15 hover:text-white hover:border-white/30 hover:bg-white/5 transition-all">
